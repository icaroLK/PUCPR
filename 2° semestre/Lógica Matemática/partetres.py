...
def QtdLetras():
    ...
    for caracter in expressao:
        if caracter in alf:
            if caracter not in unicas:
                unicas.append(caracter)
    return len(unicas)
...
def linhadic():
    dic = {}
    for linha in range(1, QtdLinhas+1):
        dic[linha] = [" "] * len(separarExpressao())



        for posCaracAnalis, carac in enumerate(separarExpressao()): #aqui arruma cada linha, deixa os F e V certinho e coloca o | e ()
            if carac in unicas:
                dic[linha][posCaracAnalis] = VouF(unicas.index(carac), linha)
            elif carac == '|' and carac not in ('(', ')'):
                dic[linha][posCaracAnalis] = '|'
            elif carac == '(':
                dic[linha][posCaracAnalis] = '('
            elif carac == ')':
                dic[linha][posCaracAnalis] = ')'
